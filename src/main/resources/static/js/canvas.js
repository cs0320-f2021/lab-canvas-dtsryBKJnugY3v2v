...
const paintToggle = () => {
  ...
  for (let i = 0; i < TOGGLE_ROWS; i++) {
    ...
    ctx.lineTo(canvas.width, i*TILE_HEIGHT);

  }
  for (let i = 0; i < TOGGLE_COLS; i++) {
    ctx.moveTo(i*TILE_WIDTH, 0);
    ctx.lineTo(i*TILE_WIDTH, canvas.height);
  }
  ctx.stroke();
  ...
  for (let row = 0; row < TOGGLE_ROWS; row++) {
    for (let col = 0; col < TOGGLE_COLS; col++) {
      ...
      // with ctx.fillText()
      ctx.fillStyle = "#F958DE";
      ctx.fillText(text, text, text);
...
const paintOnClick = event => {
  ...
  if (col == 1) {
    ...
    // 'selection' array, so be sure to check for that first!
    const index = selection.indexOf(text);
    if (index > -1) {
